...
impl CtapHidChannel for TockCtapHidChannel {
    fn send_or_recv_with_timeout(
        ...
        let interface = match self.transport {
            Transport::MainHid => usb_ctap_hid::UsbInterface::MainHid,
            #[cfg(feature = "vendor_hid")]
            Transport::VendorHid => usb_ctap_hid::UsbInterface::VendorHid,
        };
        match usb_ctap_hid::send_or_recv_with_timeout(buf, Duration::from_ms(timeout), interface) {
            Some(usb_ctap_hid::SendOrRecvStatus::Error) => Err(SendOrRecvError),
            Some(usb_ctap_hid::SendOrRecvStatus::Sent) => Ok(SendOrRecvStatus::Sent),
            Some(usb_ctap_hid::SendOrRecvStatus::Received(_)) => Ok(SendOrRecvStatus::Received),
            None => Ok(SendOrRecvStatus::TimedOut),
        }
    }
}
